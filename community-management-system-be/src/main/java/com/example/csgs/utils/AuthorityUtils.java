...
public class AuthorityUtils {
    public static boolean authority(HttpServletRequest request, String id, RedisUtils redisUtils) {
        String token = request.getHeader("csgs_token");
        if (!"".equals(token)) {
            Claims claims = JwtUtils.parserToken(token);  //解析token
            if (claims != null) {
                String uid = String.valueOf(claims.get("id"));
                String sessionID = request.getHeader("Authorization");
                Subject subject = SecurityUtils.getSubject();

                String tokenId = (String) redisUtils.get(token);
